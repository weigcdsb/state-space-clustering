...
SigbA0_f = @(k) eye(p*(1+p*k))*0.25;
SigbA0 = SigbA0_f(nClus);

Psi0 = eye(p)*1e-4;
nu0 = p+2;

% optimize d & C, ignore the cluster assignment
mubA0Mar = mubA0_mat(1:p, [1; (1:p)'+1]);
mubA0Mar = mubA0Mar(:);
d0 = zeros(N,1);
C0 = reshape(normrnd(0,1e-2,N*p,1), [], p);

[d_fit, C_fit, llhd_trace] =...
    dC_singClus(Y,d0,C0,...
    lsqr(C0,(log(mean(Y(:,1:10),2))-d0)),Q0(1:p,1:p),zeros(p,1),...
    eye(p),eye(p)*1e-4,...
    Sigx00(1:p, 1:p),mux00(1:p),zeros(p+1,1),...
    sparse(eye(p+1)*1e-2),SigbA0_f(1),mubA0Mar,Psi0,nu0,... % priors
    10, 1e-2); 
...
for k = 1:length(Lab)
    C_trans_tmp(k, ((Lab(k)-1)*p+1):(Lab(k)*p)) = C_fit(k,:);
end
...
for g = 2:ng
    ...
    [muX,~,~] = ppasmoo_poissexp_v2(Y,C_trans_tmp,d_fit,x0_tmp,Q0,A_tmp,b_tmp,Q_tmp);
    hess_tmp = hessX(muX(:), d_fit, C_trans_tmp, Q0, Q_tmp, A_tmp, Y);
    % tic;
...
colorbar()
subplot(1,2,2)
imagesc(exp(C_trans_tmp*mean(X_fit(:,:,idx), 3) + d_fit))
set(gca,'CLim',cLim)
title('fit')
colorbar()
