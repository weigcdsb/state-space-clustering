function [theta_b, epsilonOut, log_pdf] =...
    ...
    prior, T, p, obsIdx, active, density, OPTDC_tmp)

% for debug
% c=j;
% obsIdx = obsIdx;
% Y_tmp = Y(obsIdx,:);
% theta_a = THETApre{p,g-1}(c);
% theta_b = THETApre{p,g}(c);
% active = true;
% density = false;
...
if(sum(isnan(mudXvec)) ~= 0)
    ...
    try
        dX_tmp = ppasmoo_poissexp_v2(Y_tmp,[ones(N_tmp,1) theta_a.C],...
            zeros(N_tmp,1),prior.theta0,prior.Q0,...
