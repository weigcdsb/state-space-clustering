...
for k = 1:nClus
    THETA{1}(k) = sample_prior_new_chol(prior, N, T, p, true, Inf);
end
...
for g = 2:ng
    ...
    for j = 1:t_fit(g-1)
        ...
        [THETA{g}(c), epsilon(obsIdx), log_pdf] =...
            update_cluster_new_chol(Y(obsIdx,:),THETA{g-1}(c),THETA{g}(c),...
    ...
    for ii = 1:N
        ...
        if(numClus_fit(c,g) > 0)
            c_prop = c_next;
            THETA{g}(c_prop) = sample_prior_new_chol(prior, N, T, p, false, Inf);
        else
            c_prop = c;
