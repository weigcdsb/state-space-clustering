...
for k = 1:t_max+3
    THETA{1}(k) = sample_prior3(prior, N, T, p, true);
end
...
for g = 2:ng
    ...
    for ng = 1:nIn
        for j = 1:t_fit(g-1)
            ...
            [THETA{g}(c), epsilon(obsIdx), log_pdf] =...
                update_cluster2(Y(obsIdx,:),THETA{g}(c),THETA{g}(c),...
    ...
    for ii = 1:N
        ...
        if(numClus_fit(c,g) > 0)
            c_prop = c_next;
            THETA{g}(c_prop) = sample_prior3(prior, N, T, p, true);
        else
            c_prop = c;
        ...
        for j = 1:t_fit(g)
            ...
            dTmp = THETA{g}(cc).mudc(1);
            CTmp = THETA{g}(cc).mudc(2:end)';
            lamTmp = exp(CTmp*THETA{g}(cc).X + dTmp);
            logMar = sum(log(poisspdf(Y(ii,:), lamTmp))) +...
                log(mvnpdf(THETA{g}(cc).mudc,...
                THETA{g}(cc).mudc, THETA{g}(cc).Sigdc)) +...
                (p+1)*log(2*pi)/2 + log(det(THETA{g}(cc).Sigdc))/2;
            log_p(j) = logNb(numClus_fit(cc,g)) + logMar;
        end
        
        dTmp = THETA{g}(c_prop).mudc(1);
        CTmp = THETA{g}(c_prop).mudc(2:end)';
        lamTmp = exp(CTmp*THETA{g}(c_prop).X + dTmp);
        logMar = sum(log(poisspdf(Y(ii,:), lamTmp))) +...
            log(mvnpdf(THETA{g}(c_prop).mudc,...
            THETA{g}(c_prop).mudc, THETA{g}(c_prop).Sigdc)) +...
            (p+1)*log(2*pi)/2 + log(det(THETA{g}(c_prop).Sigdc))/2;
