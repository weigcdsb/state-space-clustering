function [XOut,x0Out,dOut,COut,...
    ...
    blockDiag_gibbsLoop_MM_norm(Y,X_tmp, Z_tmp, d_tmp, C_tmp,... % cluster-invariant
    mudc_tmp, Sigdc_tmp,...
    x0_tmp, b_tmp, A_tmp, Q_tmp, kMM,... % cluster-related
    Q0, mux00, Sigx00, deltadc0, Taudc0,Psidc0,nudc0,...
    BA0_all, Lamb0_f, Psi0,nu0, g, optdc, epsilon, burnIn) % priors
...
for l = uniZsort_tmp(:)'
    ...
    % (4)update Q
    Y_BA = XOut(latentId,2:T)';
    X_BA = [ones(T-1,1) XOut(latID,1:(T-1))'];
    
    BA0 = BA0_all([1; latID+1],latentId);
    ...
    BAvec = mvnrnd(BAn(:), kron(QOut(latentId,latentId), inv(Lambn)))';
    BAsamp = reshape(BAvec,[], p)';
    bOut(latentId) = BAsamp(:,1);
    AOut(latentId, latID) = BAsamp(:,2:end);
end
