function [theta_b, epsilonOut, log_pdf] =...
    update_cluster_new_na(Y_tmp,theta_a,theta_b,...
    prior, N, T, p, obsIdx, active, density, OPTDC_tmp, Y)

% for debug
% c=j;
% obsIdx = obsIdx;
% Y_tmp = Y_train(obsIdx,:);
% theta_a = THETA{g-1}(c);
% theta_b = THETA{g}(c);
% active = true;
% density = false;
...
% (1) update X_fit
gradHess = @(vecdX) gradHessX_na(vecdX, zeros(N_tmp,1),...
    [ones(N_tmp,1) theta_a.C(obsIdx,:)], prior.theta0, prior.Q0,...
...
if active
    for i = 1:N_tmp
        idxTmp =  find(~isnan(Y_tmp(i,:)));
        
        X_tmpC = theta_b.X(:,idxTmp)';
        lamC = @(c) exp(theta_b.d(idxTmp)' + X_tmpC*c);
        
        % use NUTS
        lpdf = @(c) sum(log(poisspdf(Y_tmp(i,idxTmp)', lamC(c)))) +...
            log(mvnpdf(c, prior.muC0, prior.SigC0));
        glpdf = @(c) X_tmpC'*(Y_tmp(i,idxTmp)' - lamC(c)) -...
            prior.SigC0\(c - prior.muC0);
