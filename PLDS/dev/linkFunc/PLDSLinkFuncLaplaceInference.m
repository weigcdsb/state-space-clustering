...
for tr=1:Trials
  ...
  if computeVarBound 
    ...
    % determine D_KL[q||p]
    seq(tr).varBound = 0.5*logdet(Lambda,'chol')-0.5*vec(seq(tr).posterior.xsm-Mu)'*Lambda*vec(seq(tr).posterior.xsm-Mu);
    seq(tr).varBound = seq(tr).varBound-0.5*logdet(Hessian,'chol')-0.5*trace(Lambda/Hessian)+T*xDim/2; 
